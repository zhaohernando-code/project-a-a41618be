...
[2026-04-14T22:30:16+08:00] Research recommendation:
结论
- 研发与回测主路线采用 `Tushare Pro + 巨潮资讯/交易所公告 + Qlib`，其中 `AkShare` 只作为辅助回填和原型验证工具，不作为一期对外数据许可基座。
- 一期建议模型不做“单点价格预测”，改为面向 10/20/40 个交易日的横截面收益排序和风险分层；单票建议由结构化分数、事件证据和风险阈值联合生成。
- 新闻链路一期优先采用官方公告事件流；若要覆盖媒体新闻、研报摘录或更广资讯，必须升级商业授权数据源，不能依赖抓取聚合站。

约束
- 免费与社区数据源可以支撑研发、历史回测和极小范围受控验证，但不应默认视为可合法支撑对外内测、商业化或强决策支持交付。
- 证据化数据底座必须内置 `license_tag`、`usage_scope`、`redistribution_scope`、`source_uri` 和 `lineage_hash`，把授权边界变成系统字段，而不是口头约定。

升级门槛
- 一旦存在外部测试用户、付费计划、分钟级刷新、媒体新闻展示或更高 SLA 诉求，即进入商业数据审批门，需要评估 `Choice / Wind / Datayes / 巨潮数据服务` 等企业级方案。
